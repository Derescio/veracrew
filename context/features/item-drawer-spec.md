...
- Update the dashboard page to allow for item creations as well
- In the Drawer Action bar add Favorite (star icon, yellow when active), Pin, Copy, Edit (pencil icon), and Delete (trash icon, right-aligned) — see screenshot for layout
...
- Full item detailfor the drawer component(content, collections, language, etc.) is fetched on click via API route (`/api/items/[id]`)
